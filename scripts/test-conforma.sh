...
usage() {
  cat <<EOF
Usage: test-conforma.sh -a <app> [options]

Run EC validation against a Konflux snapshot.

Options:
  -a, --application APP   Konflux application name (required, e.g. rhoai-v3-4)
...
All options can also be set via environment variables:
  APPLICATION, SNAPSHOT, POLICY_FILE, RESULTS_FILE, WORKERS, PUBKEY, VERBOSE
EOF
}
